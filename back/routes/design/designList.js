...
exports.designList_newversion = (req, res, next) => {
  const { page, cate1, sort="update", keyword:_keyword } = req.params;
  const keyword = _keyword === 'null' ? null : _keyword
let sql3 = `
SELECT 
D.uid, D.user_id, D.title, D.thumbnail, D.parent_design, 
D.parent_design AS test, 
D.category_level1, D.design_type, D.category_level2, D.category_level3, 
D.create_time, D.update_time, D.is_public, D.is_project, 
C.like_count, C.member_count, C.card_count, C.view_count, 
F.children_count, 
U.nick_name 
FROM design D 
LEFT JOIN (SELECT DD.parent_design, COUNT(*) AS children_count FROM design DD group by DD.parent_design) F ON F.parent_design = D.uid
LEFT JOIN design_counter C ON C.design_id = D.uid JOIN user U ON U.uid = D.user_id
WHERE D.parent_design IS NULL AND category_level1 = ${cate1}
${(keyword !== null )? " AND D.title LIKE \"%" + keyword + "%\""  :""}
ORDER BY ${ sort === "update" ? "D.update_time" : sort === "create" ? "D.create_time" : "C.like_count"} DESC
  LIMIT ${ page * 10}, 10;`;
  req.sql = sql3;
  next();
