...
const getParentDesignWithDesignId = (design_id) => 
  new Promise((resolve, reject) => {
    const sql = `SELECT * FROM opendesign.design WHERE uid IN (SELECT parent_design FROM opendesign.design WHERE uid=${design_id})`;
...
const getUserIdDesignId = design_id => {
  return new Promise((resolve, reject) => {
    connection.query(
      `SELECT user_id FROM design WHERE uid = ${design_id}`,
      (err, row) => {
        if (!err && row.length === 0) {
          resolve(null);
...
exports.acceptForkDesign = async (req, res, next) => {
  const { id, } = req.params;
  const { uid: user_id } = req.decoded; 

  const acceptDesign = ()=> 
    new Promise((resolve, reject) => {
    const sql = `UPDATE opendesign.design SET d_flag = 1 WHERE uid = ${id}`;
  ...
  const notify = async() => {
    const p_design = await getParentDesignWithDesignId(id); 
    const receiver = await getUserIdDesignId(id);
    const { sendAlarm } = require("../../socket");
    await getSocketId(receiver)
    // sendAlarm = (socketId, uid, contentId, message, fromUserId, subContentId)
      .then(socket => {
        sendAlarm(socket.socketId, receiver, id, "ACCEPTED", user_id, p_design.uid);
      });
    return true;
  };

  const respond = (data) => { res.status(200).json({ success: true, }); };
  const reject = (e) => { console.error(e); res.status(400).json({success: false, detail: e});};
...
exports.kickoutForkDesign = async (req, res, next) => {
  const { id } = req.params;
  const { uid: user_id } = req.decoded;
  const receiver = await getUserIdDesignId(id);
  const p_design = await getParentDesignWithDesignId(id); 

// console.log("kickout fork design:", id, user_id, receiver, p_design);
// kickout fork design: 5542 1950 1949 RowDataPacket {
//    uid: 5490,
//    user_id: 1950,
//    title: '80\'s',
//    explanation: null,
...
//    is_public: 1,
//    is_modify: 0,
//    is_display_creater: 0,
//    is_problem: 0,
//    parent_design: null,
//    create_time: 2023-07-21T03:33:42.000Z,
//    update_time: 2023-07-21T03:33:42.000Z,
//    is_project: 0,
//    is_members: 1,
//    d_flag: 0 }
  const deleteDesign = () => 
  ...
  const notify = async() => {
    ...
    await getSocketId(receiver)
      .then(socket => {
        sendAlarm(socket.socketId, receiver, p_design.uid, "KICKOUT", user_id, id);
      });
    return true;
  };
...
exports.denyForkDesign = async (req, res, next) => {
  const { id } = req.params;
  const { uid: user_id } = req.decoded;
  const receiver = await getUserIdDesignId(id);
  const p_design = await getParentDesignWithDesignId(id); 
  ...
  const notify = async() => {
    ...
    await getSocketId(receiver)
      .then(socket => {
        sendAlarm(socket.socketId, user_id, id, "DENIDED", receiver, p_design.uid);
      });
    return true;
  };
...
exports.forkDesign2 = async (req, res, next) => {
  ...
  function cloneDesignDB(parent) {
    return new Promise((resolve, reject) => {
      const sql = `
      INSERT INTO opendesign.design(user_id, title, explanation, parent_design, category_level1, design_type, category_level2, category_level3, is_commercial, is_public, is_modify, is_problem, is_display_creater, is_project)
      SELECT ${user_id} AS user_id, title, explanation, ${parent} as parent_design, category_level1, design_type, category_level2, category_level3, is_commercial, is_public, is_modify, ${false}, is_display_creater, is_project
...
.then(async newDesignId => {  

const getUserIdParentDesign = () => 
new Promise((resolve, reject) => {
const sql = `SELECT user_id FROM opendesign.design WHERE uid=${parent};`;//`SELECT user_id FROM opendesign.design WHERE uid IN (SELECT parent_design FROM opendesign.design WHERE uid=${parent})`;
connection.query(sql, (err, rows) => {
if(!err) {
resolve(rows?rows[0].user_id:null);
} else {
console.error(err);
reject(err);
}
...
const parent_uid = await getUserIdParentDesign(parent);
console.log("notify", parent_uid);
if(parent_uid === null) return;
const { sendAlarm } = require("../../socket");
await getSocketId(parent_uid)//.then(console.log);
.then(socket => {
sendAlarm(
socket.socketId, 
parent_uid, 
...
.catch(err => 
res.status(200).json({ data: "failed: " + err }));
  ...
  function getChildDesign(id) {
    ...
    return new Promise((resolve, reject) => {
      connection.query(
        ...
        `SELECT 
        D.uid, D.user_id, D.title, D.d_flag, 
        U.nick_name, 
        T.m_img , T.s_img,
