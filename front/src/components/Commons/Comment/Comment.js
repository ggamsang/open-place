...
class Comment extends Component {
  ...
  undoReply() {
    this.setState({ reply: false, this_reply: "" });
  }
  undoComment() {
  ...
  async removeComment(commentId) {
    if (
      (await confirm("선택하신 댓글을 삭제하시겠습니까?", "예", "아니오")) ===
      false
    ) {
      return;
    }
    const comm = this.props.comments.find((comm) => {
      return comm.uid === commentId;
    });
  ...
  render() {
    ...
    const myface =
      my && my.thumbnail && my.thumbnail.s_img !== null
        ? my.thumbnail.s_img
        : noface;
            ...
            return (
              <Comments key={item.nick_name + index}>
                ...
                <CommentInner
                  face={face}
                  className={
                    !disabledBlink && item.read === 0 ? "blinking" : ""
                  }
                >
                  <div className="face" />
                  <div className="text-wrapper">
                    <div className="nick">
                      ...
                      <div className="button-wrapper">
                        {!disabledReply && !reply && (
                          <div
                            onClick={() => this.reply(item.uid)}
                            className="reply"
                          >
                            답글달기
                          </div>
                        )}
                        {my && my.uid === item.user_id && (
                          <div
                            onClick={() => this.removeComment(item.uid)}
                            className="reply del"
                          >
                            삭제하기
                          </div>
                ...
                {item.replies &&
                  ...
                  item.replies.map((repli, repli_index) => {
                    const repli_face =
                      repli && repli.s_img !== null ? repli.s_img : noface;
                    return (
                      <CommentInner
                        className={
                          !disabledBlink && repli.read === 0
                            ? "reply blinking"
                            : "reply"
                        }
                        key={repli.nick_name + repli.uid + repli_index}
                        face={repli_face}
                      >
                        <div className="face" />
                        <div className="text-wrapper">
                          ...
                          <div
                            className="comment"
                            dangerouslySetInnerHTML={{ __html: repli.comment }}
                          ></div>
                        </div>
                        <div className="button-wrapper">
                          {my && my.uid === repli.user_id && (
                            <div
                              onClick={() => this.removeReply(repli.uid)}
                              className="del"
                            >
                              삭제하기
                            </div>
                  ...
                  <React.Fragment>
                    ...
                    >
                      <div className="writeBox">
                        {this.state.ing ? "답글 다는 중..." : my.nickName}
                      </div>
                      <div className="flex_Input">
                        ...
                        <div className="another-wrapper">
                          <div
                            className="submit"
                            onClick={() => this.requestReply(item.uid)}
                          >
                            게시
                          </div>
