...
const FlexContainer = styled.div`
  ...
  margin-left: auto;
  margin-right: auto;
  @media only screen and (min-width: ${osdcss.newresolutions
      .mini}px) and (max-width: ${osdcss.newresolutions.small}px) {
    width: ${(props) =>
      props.type == "design"
        ? "720px"
        : props.type == "group"
  ...
  @media only screen and (min-width: ${osdcss.newresolutions
      .small}px) and (max-width: ${osdcss.newresolutions.medium}px) {
    width: ${(props) =>
      props.type == "design"
        ? "1090px"
        : props.type == "group"
  ...
  @media only screen and (min-width: ${osdcss.newresolutions
      .medium}px) and (max-width: ${osdcss.newresolutions.large}px) {
    width: ${(props) =>
      props.type == "design"
        ? "1200px"
        : props.type == "group"
  ...
  // @media only screen and (min-width : ${osdcss.resolutions
    .SmallMinWidth}px) and (max-width : ${osdcss.resolutions.SmallMaxWidth}px) {
  //   width: 330px;
  // }
  // @media only screen and (min-width : ${osdcss.resolutions
    .MediumMinWidth}px) and (max-width : ${osdcss.resolutions
    .MediumMaxWidth}px) {
  //   margin-left: ${(props) =>
    props.type === "design" ? 100 : props.type === "group" ? 215 : 67}px;
  //   width: ${osdcss.resolutions.MediumMaxWidth}px;
  // }
  // @media only screen and (min-width : ${osdcss.resolutions
    .LargeMinWidth}px) and (max-width : ${osdcss.resolutions.LargeMaxWidth}px) {
  //   margin-left: ${(props) =>
    props.type === "design" ? 9 : props.type === "group" ? 11 : 12}px;
  ...
  getColumnNumber(type) {
    ...
    let w =
      window.innerWidth > osdcss.resolutions.LargeMaxWidth
        ...
        : window.innerWidth;
    if (
      osdcss.resolutions.SmallMinWidth <= w &&
      w < osdcss.resolutions.SmallMaxWidth
    ) {
      cols =
        type === "design"
          ? osdcss.design_margin.small.cols
          : type === "group"
          ? osdcss.group_margin.small.cols
          : osdcss.designer_margin.small.cols;
    } else if (
      osdcss.resolutions.MediumMinWidth <= w &&
      w < osdcss.resolutions.MediumMaxWidth
    ) {
      cols =
        type === "design"
          ? osdcss.design_margin.medium.cols
          : type === "group"
          ? osdcss.group_margin.medium.cols
          : osdcss.designer_margin.medium.cols;
    } else if (
      osdcss.resolutions.LargeMinWidth <= w &&
      w < osdcss.resolutions.LargeMaxWidth
    ) {
      cols =
        type === "design"
          ? osdcss.design_margin.large.cols
          : type === "group"
          ? osdcss.group_margin.large.cols
          : osdcss.designer_margin.large.cols;
    } else {
      cols =
        type === "design"
          ? osdcss.design_margin.big.cols
          : type === "group"
          ? osdcss.group_margin.big.cols
    ...
    return dataListAdded && dataListAdded.length > 0 ? (
      ...
      >
        {dataListAdded.map((item, i) => {
          // console.log(item);
          const last =
            ((i + 1) % cols === 0 && i !== 0) || (cols === 1 && i === 0)
              ? "right-last"
              : "";
          const bottom =
            dataListAdded.length - (dataListAdded.length % cols) - 1 < i ||
            dataListAdded.length - cols === 0
              ? "bottom-last"
              : "";
          return (
            <FlexBox
              ...
              marginRightLast={marginRightLast}
              marginBottomLast={marginBottomLast}
              key={i}
