...
class DesignComment extends React.Component {
  ...
  createCommentRequest = async (data) => {
    this.props
      .CreateDesignCommentRequest(data, this.props.id, this.props.token)
      .then((res) => {
        this.getComment();
      })
  ...
  onSubmitCmtForm = async (data) => {
    ...
    if (
      !this.state.reply &&
      (packet.comment.length === 0 || packet.comment.trim() === "")
    ) {
  ...
  onDeleteComment = async (data) => {
    ...
    this.props
      .DeleteDesignCommentRequest(this.props.id, data.uid, this.props.token)
      .then((res) => {
  ...
  deleteComment = (id) => {
    this.props
      .DeleteDesignCommentRequest(this.props.id, id, this.props.token)
      .then((res) => {
        if (res.data && res.data.success === true) {
          this.getComment();
        }
    ...
    const CommentForm = (value) => {
      return (
        ...
        >
          <FormField
            name="comment"
            handleOnBlur={this.commentFormBlur}
            value={value.toWhom}
            RenderComponent={FormTextAreaRed}
            maxLength="1000"
          />
        ...
        <Modal.Content>
          ...
          <CommentContainer onClick={this.checkBlur}>
                  ...
                  <li key={comm.uid} style={{ marginTop: "25px" }}>
                    ...
                    >
                      <div
                        ...
                        onClick={this.onClickedReply(comm.uid, comm.nick_name)}
                      >
                        {comm.nick_name}
                      </div>
                            ...
                            >
                              <div
                                style={{
                                  ...
                                  width: "35px",
                                  height: "35px",
                                  backgroundImage: `url(${reply.s_img}), url(${logo})`,
                                  backgroundPosition: "center",
                                  backgroundSize: "cover",
                                }}
                              />
            ...
            {this.state.render ? (
              <div>
                <br />
                <CommentForm parent={null} toWhom={null} />
              </div>
            ) : null}
          </CommentContainer>
        </Modal.Content>
