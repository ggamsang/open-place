...
class NewCardModal extends Component {
  ...
  submit = async (obj) => {
    ...
    await ValidationGroup(this.state, false).then(async (data) => {
        ...
        .then(async (res) => {
          if (res.success) {
            ...
            this.props
              ...
              .catch((err) =>
                alert(err + "와 같은 이유로 작업을 완료할 수 없습니다.")
              );
          } else {
            await alert(
              "새로운 카드를 추가하는데 실패했습니다. 잠시후 다시 시도해주세요.",
              "확인"
            );
          }
        });
