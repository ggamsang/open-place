...
const AsBelowArrow = styled.div`
  ...
  border-top: ${(props) => props.percent * 65}px solid
    ${(props) => props.color || "#707070"};
...
const Container = SortableContainer(({ children }) => {
  return <ul style={{ margin: "0px", padding: "0px" }}>{children}</ul>;
});
...
const SortableCard = SortableElement(
  ({ editor, card, openCard, boardId, design_id, userInfo }) => (
    ...
    <HorizonBox>
      <div className="bound_box">
        ...
        {editor && (
          <div
            style={{
              marginTop: step.cards && step.cards.length > 0 ? "0px" : "66px",
            }}
          >
            ...
            <CreateCard
              onClick={() => createCard(step.order, boardId)}
              title={""}
              step={"카드 "}
              marginTop={0}
              marginRight={0}
...
class SortableDesignCards extends Component {
  ...
  render() {
    ...
    const { editor, design_id, openCard, createCard, boardId, userInfo } =
      this.props;
...
class SortableDesignSteps extends Component {
  ...
  render() {
    const { items } = this.state;
    const { editor, design_id, cardReorder, createCard, openCard, userInfo } =
      this.props;
    return (
      <Container
        axis="x"
