...
const MsgSectionBoard = styled.div`
  width: 100%;
  height: ${(props) => props.height + "px" || "98%"};
  padding-top: 25px;
  // padding-right: 10px;
  position: relative;
    ...
    <ReceiveMessageBox>
      <div className="messageReceiveBubble">
        <div
          ...
          dangerouslySetInnerHTML={{ __html: props.msgText }}
        >
          {/* {props.msgText} */}
        </div>
      </div>
...
function LoadMessage(props) {
  if (props.isMyMsg === true) {
    ...
    return (
      <MsgReceiveBox msgText={props.msgText} updateTime={props.updateTime} />
    );
  }
}
class MessageDetail extends Component {
  ...
  getLoadData = async () => {
    ...
    await this.setState({ loading: true }, () => {
      this.props
        .GetMyMsgDetailRequest(this.props.token, this.props.id, this.state.page)
        .then(() => {
          this.setState({
            loading: false,
            page: this.state.page + 1,
            hasMore: this.checkHasMore(this.props.MessageDetail),
            addList: this.props.MessageDetail,
            nowList: this.props.MessageDetail.reverse().concat(
              this.state.nowList
            ),
          });
  ...
  handleScroll = async (e) => {
    ...
    this.setState({
      ...
      nowScroll: scrollHeight - this.state.scrollLocation,
    });
