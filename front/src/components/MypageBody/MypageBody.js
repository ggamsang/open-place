...
class MypageBody extends Component {
  ...
  getLikeDesignList = async (page) => {
    this.props.id &&
      this.props.GetLikeInDesignerRequest(
        this.props.id,
        page,
        this.state.orders[0].keyword
      );
  };
  getLikeDesignerList = async (page) => {
    this.props.id &&
      this.props.GetLikeDesignerInDesignerRequest(
        this.props.id,
        page,
        this.state.orders[2].keyword
      );
  };
  getLikeGroupList = async (page) => {
    this.props.id &&
      this.props.GetLikeGroupInDesignerRequest(
        this.props.id,
        page,
        this.state.orders[1].keyword
      );
  };
  getMyGroupListRequest = async (page) => {
    this.props.id &&
      this.props.GetGroupInDesignerRequest(
        this.props.id,
        page,
        this.state.this_order.keyword
      );
  };
  getMyDesignListRequest = async (page) => {
    this.props.id &&
      this.props.GetMyDesignInDesignerRequest(
        this.props.id,
        page,
        this.state.this_order.keyword
      );
  ...
  getRelatedGroupInDesignerRequest = async (page) => {
    this.props.id &&
      this.props.GetRelatedGroupInDesignerRequest(
        this.props.id,
        page,
        this.state.this_order.keyword
      );
  ...
  render() {
    ...
    const careerList =
      (MyDetail &&
        MyDetail.careerlist &&
        MyDetail.careerlist.split("/").slice(0, -1)) ||
      [];
      ...
      <Wrapper>
        <div className="menu-container">
          <TabMenu>
            ...
            <a onClick={() => this.changeCategory(0)}>
              <div
                className={`tab ${
                  this.state.cateIndex === 0 ? "selected" : ""
                }`}
              >
                그룹({NumberFormat(Count.total_group || 0)})
              </div>
            </a>
            <a onClick={() => this.changeCategory(1)}>
              <div
                className={`tab ${
                  this.state.cateIndex === 1 ? "selected" : ""
                }`}
              >
                참여그룹({NumberFormat(Count.joined_group || 0)})
              </div>
            </a>
            <a onClick={() => this.changeCategory(2)}>
              <div
                className={`tab ${
            ...
            <a onClick={() => this.changeCategory(3)}>
              <div
                className={`tab ${
                  this.state.cateIndex === 3 ? "selected" : ""
                }`}
              >
                관심항목({NumberFormat(Count.total_favorite || 0)})
              </div>
            </a>
            <a onClick={() => this.changeCategory(4)}>
              <div
                className={`tab ${
                  this.state.cateIndex === 4 ? "selected" : ""
                }`}
              >
                경험
              </div>
          ...
          {this.state.cateIndex === 0 && (
            <OrderOption
              ...
              order_clicked={(order) =>
                this.handleChangeOrderOps(order, this.getMyGroupListRequest)
              }
              selected={this_order}
            />
          )}
          {this.state.cateIndex === 1 && (
            <OrderOption
              style={{ marginBottom: "15px" }}
              order_clicked={(order) =>
                this.handleChangeOrderOps(
                  order,
                  this.getRelatedGroupInDesignerRequest
                )
              }
              selected={this_order}
            />
          ...
          {this.state.cateIndex === 2 && (
            <OrderOption
              style={{ marginBottom: "15px" }}
              order_clicked={(order) =>
                this.handleChangeOrderOps(order, this.getMyDesignListRequest)
              }
              selected={this_order}
            />
          )}
          ...
          <div style={{ paddingTop: "37px" }}>
            {this.props.status === "INIT" ? (
              ...
              <ScrollList
                ...
                type="group"
                dataList={MyGroup}
                dataListAdded={MyGroupAdded}
                getListRequest={this.getMyGroupListRequest}
              />
            )}
          </div>
        )}
          ...
          <div style={{ paddingTop: "37px" }}>
            {this.props.status === "INIT" ? (
              ...
              <ScrollList
                {...opendesign_style.group_margin}
                type="group"
                dataList={RelatedGroup}
                dataListAdded={RelatedGroupAdded}
                getListRequest={this.getRelatedGroupInDesignerRequest}
              />
            )}
          </div>
        )}
          ...
          <div style={{ paddingTop: "37px" }}>
            {this.props.status === "INIT" ? (
              ...
              <ScrollList
                ...
                type="design"
                dataList={MyDesign}
                dataListAdded={MyDesignAdded}
                getListRequest={this.getMyDesignListRequest}
              />
            )}
          </div>
        )}
        ...
        {this.state.cateIndex === 3 && (
          <div style={{ paddingTop: "37px" }}>
              ...
              <React.Fragment>
                ...
                {this.props.status === "INIT" ? (
                  ...
                  <ScrollList
                    ...
                    {...opendesign_style.design_margin}
                    type="design"
                    dataList={MyLikeDesign}
                    dataListAdded={MyLikeDesignAdded}
                    getListRequest={this.getLikeDesignList}
                  />
                )}
              </React.Fragment>
            ) : null}
            {Count.like_group > 0 ? (
              <React.Fragment>
                <div className="interested">
                  관심있는 그룹({NumberFormat(Count.like_group)})
                </div>
                {this.props.status === "INIT" ? (
                  <Loading />
                ) : (
                  <ScrollList
                    ...
                    {...opendesign_style.group_margin}
                    type="group"
                    dataList={MyLikeGroup}
                    dataListAdded={MyLikeGroupAdded}
                    getListRequest={this.getLikeGroupList}
                  />
                )}
              </React.Fragment>
            ) : null}
            ...
            {Count.like_designer > 0 ? (
              <React.Fragment>
                <div className="interested">
                  관심있는 디자이너({NumberFormat(Count.like_designer)})
                </div>
                {this.props.status === "INIT" ? (
                  <Loading />
                ) : (
                  <ScrollList
                    ...
                    type="designer"
                    dataList={MyLikeDesigner}
                    dataListAdded={MyLikeDesignerAdded}
                    getListRequest={this.getLikeDesignerList}
                  />
                )}
              </React.Fragment>
            ) : null}
