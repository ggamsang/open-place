...
class DesignerListContainer_mobile extends Component {
  ...
  componentDidMount() {
    ...
    this.props.GetDesignerListRequest(
      0,
      this.props.sort,
      this.props.cate1,
      this.props.cate2,
  ...
  async handleChangeCategory(category) {
    ...
    window.location.href =
      "/designer" + `/${orderkeyword}` + "/" + category.value;
  }
  async handleChangeSubCategory(parent, category) {
    ...
    this.props.GetDesignerTotalCountRequest(
  ...
  handleCreateDesigner() {
    let href = window.location.href.substring(
      0,
      window.location.href.search("designer")
    );
