...
import ReviewList from "mobile/components_mobile/Review/ReviewList";
import {
  GetExpReviewListRequest,
  DeleteExpReviewRequest,
} from "actions/Review";
...
class ReviewContainer extends React.Component {
  componentDidMount() {
    ...
    //   this.props.ExpDetail.length !== 0 &&
    this.props.GetExpReviewListRequest(this.props.DesignDetail.uid);
  }
  modifyReview = (id, data) => {};
  deleteReview = (id) => {
    const { token, DesignDetaild: ExpDetail } = this.props;
    ...
    if (rconfirm)
      this.props
        ...
        .then(
          (data) =>
            data.success && this.props.GetExpReviewListRequest(ExpDetail.uid)
        );
  };
  render() {
    return (
      <ReviewList
        ...
        onCloseWriteModal={() => this.props.onCloseWriteModal}
        {...this.props}
        ExpDetail={this.props.DesignDetail}
