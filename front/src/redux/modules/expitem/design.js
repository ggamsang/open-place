...
const DesignNotFound = () => ({
  ...
  DesignDetail: { status: DESIGN_NOT_FOUND },
});
const CreateDesignSuccess = (res) => ({
...
const DeleteDesignSuccess = (res) => ({
  type: DELETE_DESIGN_SUCCESS,
  success: res.success,
});
const DeleteDesignFailure = (error) => ({
  type: DELETE_DESIGN_FAILURE,
  success: error.success,
});
const UpdateDesignInfo = () => ({ type: UPDATE_DESIGN_INFO });
const UpdateDesignInfoSuccess = (res) => ({
  type: UPDATE_DESIGN_INFO_SUCCESS,
  res,
});
const UpdateDesignInfoFailure = (error) => ({
  type: UPDATE_DESIGN_INFO_FAILURE,
  data: error,
});
const ForkDesign = () => ({ type: FORK_DESIGN });
const ForkDesignSuccess = (new_design_id) => ({
  type: FORK_DESIGN_SUCCESS,
  new_design_id,
});
...
const ForkDesignListSuccess = (list) => ({
  type: FORK_DESIGN_LIST_SUCCESS,
  list,
});
const ForkDesignListFailure = () => ({ type: FORK_DESIGN_LIST_FAILURE });
const DesignWaitingList = () => ({ type: GET_WAITING_LIST });
const DesignWaitingListSuccess = (data) => ({
  type: GET_WAITING_LIST_SUCCESS,
  list: data,
});
const DesignWaitingListFailure = () => ({
  type: GET_WAITING_LIST_FAILURE,
  list: [],
});
const DesignWaitingToAcceptList = () => ({ type: GET_WAITING_TO_ACCEPT_LIST });
const DesignWaitingToAcceptListSuccess = (data) => ({
  type: GET_WAITING_TO_ACCEPT_LIST_SUCCESS,
  list: data,
});
const DesignWaitingToAcceptListFailure = () => ({
  type: GET_WAITING_TO_ACCEPT_LIST_FAILURE,
  list: [],
});
const GetDesignDetailStepCard = (data) => ({
  type: GET_DESIGN_DETAIL_STEP_CARD,
  DesignDetailStepCard: data,
});
const GetLikeDesign = () => ({ type: GET_LIKE_DESIGN });
const GetLikeDesignSuccess = (data) => ({
  type: GET_LIKE_DESIGN_SUCCESS,
  like: data,
});
const GetLikeDesignFailure = (data) => ({
  type: GET_LIKE_DESIGN_FAILURE,
  like: data,
});
...
const ChangeToProjectSuccess = (data) => ({
  type: CHANGE_TO_PROJECT_SUCCESS,
  data: data,
});
const ChangeToProjectFailure = () => ({ type: CHANGE_TO_PROJECT_FAILURE });
const GetDesignDetail = (data) => ({
  type: GET_DESIGN_DETAIL,
  DesignDetail: data,
});
const DesignDetailReset = () => ({
  type: DESIGN_DETAIL_RESET,
  DesignDetail: [],
});
const GetDesignCount = (data) => ({ type: GET_DESIGN_COUNT, Count: data });
const UpdateDesignView = () => ({ type: UPDATE_DESIGN_VIEW });
const GetDesignDetailView = (data) => ({
  type: GET_DESIGN_DETAIL_VIEW,
  DesignDetailView: data,
});
const DesignDetailViewReset = () => ({
  type: DESIGN_DETAIL_VIEW_RESET,
  DesignDetailView: [],
});
const GetDesignDetailStep = (data) => ({
  type: GET_DESIGN_DETAIL_STEP,
...
export function Design(state, action) {
  ...
  switch (action.type) {
    ...
    case FORK_DESIGN_LIST:
      return update(state, {
        status: { DesignForked: { $set: "WAIT_FORK_LIST" } },
      });
    case FORK_DESIGN_LIST_SUCCESS:
      return update(state, {
        status: {
          DesignForked: { $set: "SUCCESS_FORK_LIST" },
    ...
    case FORK_DESIGN_LIST_FAILURE:
      return update(state, {
        status: { DesignForked: { $set: "FAILURE_FORK_LIST" } },
      });
    case GET_LIKE_DESIGN:
      return update(state, { LikeDesign: { status: { $set: "WATTING" } } });
    case GET_LIKE_DESIGN_SUCCESS:
      return update(state, {
        LikeDesign: { status: { $set: "SUCCESS" } },
        status: { like: { $set: action.like } },
      });
    case GET_LIKE_DESIGN_FAILURE:
      return update(state, {
        LikeDesign: { status: { $set: "FAILURE" } },
        status: { like: { $set: action.like } },
      });
    case LIKE_DESIGN:
    ...
    case GET_DESIGN_DETAIL_VIEW:
      return update(state, {
        status: { DesignDetailView: { $set: action.DesignDetailView } },
      });
...
export function GetDesignDetailStepCardRequest(id, card_id) {
  return (dispatch) => {
    return fetch(
      `${host}/design/designDetail/` + id + "/cardDetail/" + card_id,
      {
...
export function CreateDesignRequest(data, token) {
  return (dispatch) => {
    ...
    return fetch(`${host}/design/createDesign`, {
...
export function GetoutDesignRequest(id, memberId, token, refuse) {
  return (dispatch) => {
    ...
    return fetch(
      `${host}/design/designDetail/${id}/getoutDesign/${memberId}/${refuse}`,
      {
        headers: {
          "Content-Type": "application/json",
          "x-access-token": token,
...
export function ForkDesignListRequest(design_id, token) {
  return (dispatch) => {
    ...
    return fetch(`${host}/design/forkDesignList/${design_id}`, {
      ...
      .then((data) =>
        data.success
          ? dispatch(ForkDesignListSuccess(data.list))
          : dispatch(ForkDesignListFailure())
      )
      .catch((error) => {
        console.error("err", error);
...
export function AcceptForkDesignRequest(id, token) {
  return new Promise((resolve, reject) => {
    const url = `${host}/design/forkdesign/accept/${id}`;
    fetch(url, {
      headers: { "x-access-token": token },
      method: "POST",
    })
...
export function DenyForkDesignRequest(id, token) {
  return new Promise((resolve, reject) => {
    const url = `${host}/design/forkdesign/deny/${id}`;
    fetch(url, {
      headers: { "x-access-token": token },
      method: "DELETE",
    })
