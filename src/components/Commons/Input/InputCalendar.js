...
const FormStyle = styled.input.attrs({ type: "date" })`
    ...
    padding-left: 20px;
    font: normal normal normal 13px/22px Noto Sans KR;
    letter-spacing: 0px;
    color: #000000;
...
export class InputCalendar extends Component {
  ...
  returnData = async (e) => {
    this.props.getStartDateValue &&
      (await this.props.getStartDateValue(this.state.startDate));
    this.props.getEndDateValue &&
      (await this.props.getEndDateValue(this.state.endDate));
    this.props.getDayDateValue &&
      (await this.props.getDayDateValue(this.state.dayDate));
  };
  init = async () => {
    await this.setState({
      startDate:
        this.props.startDate || new Date().toISOString().substring(0, 10),
    });
    await this.setState({
      endDate: this.props.endDate || new Date().toISOString().substring(0, 10),
    });
  ...
  async onChangeStartDate(event) {
    ...
    if (dDay < 1) {
      ...
      await this.setState({
        ...
        dayDate: Math.floor(timestamp / oneDay),
      });
    }
    this.returnData();
  }
