    ...
    Promise.all(
      files.map((file) =>
        GameFileUploadRequest(
          file,
          encodeURIComponent(
            `${folder}/${file.webkitRelativePath || "index.html"}`
          )
      ...
      .then(
        (
          list // console.log("game: changed aaa", list));
        ) => list.filter((item) => item.name === "index.html")
      )
      .then((obj) =>
        this.setState({ files: obj }, async () => await this.returnData())
      );
