...
import ScrollDesignListContainer from "containers/ExpItem/ScrollDesignListContainer";
...
class SearchListRe extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      JSON.stringify(prevProps.designs) !==
        JSON.stringify(this.props.designs) ||
      JSON.stringify(prevProps.groups) !== JSON.stringify(this.props.groups) ||
      JSON.stringify(prevProps.designers) !==
        JSON.stringify(this.props.designers)
    ) {
      ...
      if (
        this.props.design_status != "INIT" &&
        this.props.group_status != "INIT" &&
        this.props.designer_status != "INIT" &&
        this.state.update == false
      ) {
        if (designs) {
          this.setState({ update: true, selectCate: 1, urlCate: "design" });
        } else if (groups) {
          this.setState({ update: true, selectCate: 2, urlCate: "group" });
        } else if (designers) {
          this.setState({ update: true, selectCate: 3, urlCate: "designer" });
        }
      }
    }
  ...
  onChangeDropBox(_, { value }) {
    ...
    this.props.history.replace(
      `/search/${this.props.sort}/${this.props.keyword}`
    );
  }
  ...
  handleChangeSubCategory = async (parent, category) => {
    await this.setState({
      main_category: this.props.category1[parent.value - 1],
      this_category: category,
      ...
      <SearchContainer>
        ...
        <div className="result_list">
          <div className="result">
            <div
              style={{
                display: this.state.urlCate === "design" ? "block" : "none",
              }}
            >
            ...
            <div
              style={{
                display: this.state.urlCate === "group" ? "block" : "none",
              }}
            >
            ...
            <div
              style={{
                display: this.state.urlCate === "designer" ? "block" : "none",
              }}
            >
