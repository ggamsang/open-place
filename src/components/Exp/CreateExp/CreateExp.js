...
import TextAreaNormal from "components/Commons/TextArea/TextAreaNormal";
import { InputPrice } from "components/Commons/Input";
import AddContent from "components/Commons/AddContent/AddContent";
import { Editor } from "commons/Editor/Editor";
import { InputGameFile } from "components/Commons/Input";
...
class CreateExp extends React.Component {
  ...
  onChangeExpType = (event) => {
    if (parseInt(event.target.value, 10) === parseInt(ExpTypes["GAMING"], 10)) {
      console.log("DEBUG:", this.state);
      this.setState({ tag: ["게임"] });
      // this.setState({
      //   tag: this.state.tag ? [...this.state.tag, "게임"] : ["게임"],
      // });
    }
    this.setState({
          ...
          <div className="row">
            ...
            <div>
              <InputTag
                ...
                getValue={(v) => this.handleAddTag(v)}
                width={"245"}
              />
            </div>
          </div>
