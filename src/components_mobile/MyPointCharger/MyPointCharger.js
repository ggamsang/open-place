...
class MyPointCharger extends React.Component {
    ...
    onChangePrice = point => this.setState({ point: point }, () => { this.setState({ active: this.state.point > 0 && this.state.type != 0 }) });
    SelectChargeType = type => this.setState({ type: type }, () => { this.setState({ active: this.state.point > 0 && this.state.type != 0 }) });
    handleCharge = () =>
        this.props.token &&
        this.props.ChargeMyPointRequest(this.props.token, this.state.point, this.state.type)
            .then(res => {
                if (res) {
                    alert('success');
