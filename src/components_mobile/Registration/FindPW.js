...
import InputEmail from 'components_mobile/Commons/Input/InputEmail';
      ...
      return (
        ...
        <Fade when={this.state.page1}>
          ...
          <div style={{height:"50vh"}} className='box column alignCenter justifyCenter'>
              ...
              <GradientButton  onClickEvent={()=>{this.setState({page1:false});setTimeout(()=>this.setState({page2_2:true}),1000)}} style={{marginBottom:"20px"}} text="비밀번호 찾기" width={292} height={52} front={'#365AF1'} end={'#FF4343'} deg={270} radius={28}/>
          </div>
          </div>
        </Fade> 
        ...
        <Fade when={this.state.page2_2}>
          <div className='content' style={{display:`${this.state.page2_2==false?"none":"block"}`}}>
            ...
            <div style={{height:"40vh"}} className='buttonWrap justifyCenter'>
              <GradientButton  onClickEvent={()=>{
                                this.setState({page2_2:false,message:`${"임시비밀번호를 메일로 발급하였습니다"}`})
                                setTimeout(()=>{
                                  this.setState({page3:true})
                                },1000)
              }} 
