...
const ModalBoxMobile = styled(Modal)`
  min-width: 265px;
  max-width: 265px;
  min-height: 152px;
...
class Alert extends React.Component {
  render() {
    const {
      ...
      confirmation,
      show,
      proceed, // enableEscape = true
    } = this.props;

    return (
      <React.Fragment>
        {window.innerWidth < 500 ? (
          <ModalBoxMobile open={show} onClose={() => proceed(false)}>
            <div className="wrapper">
              {title ? <div className="titleBox">{title}</div> : null}
          ...
          <ModalBox open={show} onClose={() => proceed(false)}>
            {title ? <div className="titleBox">{title}</div> : null}
