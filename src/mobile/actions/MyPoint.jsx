import * as types from "../../mobile/actions/ActionTypes";
import host from "../../config";
import { authGET, POST } from "../../constants";

// ACTION CREATOR
const GetPointList = (data) => ({ type: types.GET_POINT_LIST, payload: data });
const PointListFail = () => ({ type: types.GET_POINT_LIST_FAIL, payload: [] });
const GetTotalPointCount = (data) => ({
  type: types.GET_POINT_TOTAL_COUNT,
  payload: data,
});
const PointCountFail = () => ({
  type: types.GET_POINT_COUNT_FAIL,
  payload: [],
});

const GetCurrentPoint = (data) => ({
  type: types.GET_CURRENT_POINT,
  payload: data,
});
const GetCurrentPointFailed = (e) => ({
  type: types.GET_CURRENT_POINT_FAILED,
  payload: e,
});
const ChargeMyPoint = (data) => ({
  type: types.CHARGE_MY_POINT,
  payload: data,
});
const ChargeMyPointFailed = (e) => ({
  type: types.CHARGE_MY_POINT_FAILED,
  payload: e,
});
...
export const GetPointListRequest = (page, token) => {
  return (dispatch) => {
    ...
    return fetch(url, authGET(token))
      .then((res) => res.json())
      .then((data) =>
        data.success ? dispatch(GetPointList(data.detail)) : GetPointList([])
      )
