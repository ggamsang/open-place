...
export class InputPrice extends Component {
  ...
  returnData = async (e) => {
    this.props.onChangeValue &&
      (await this.props.onChangeValue(this.state.price));
  };
  init = async () => {
  ...
  render() {
    ...
    return (
      <FormBox color={this.props.color}>
        <div className="formwrapper">
          {this.state.price != 0 ? (
            <button
              className="clear"
              onClick={() =>
                this.setState({ price: 0 }, () => this.returnData())
              }
            >
              <span>x</span>
            </button>
        ...
        <div className="buttonbox">
          <Button
            marginRight={tag && tag.marginRight}
            width={tag && tag.width}
            onClick={() => this.onClickButton(1000)}
          >
            <div className="text">+1천</div>
          </Button>
          <Button
            marginRight={tag && tag.marginRight}
            width={tag && tag.width}
            onClick={() => this.onClickButton(10000)}
          >
          ...
          <Button
            marginRight={tag && tag.marginRight}
            width={tag && tag.width}
            onClick={() => this.onClickButton(50000)}
          >
            <div className="text">+5만</div>
          </Button>
          <Button
            marginRight={tag && tag.marginRight}
            width={tag && tag.width}
            onClick={() => this.onClickButton(100000)}
          >
            <div className="text">+10만</div>
          </Button>
          <Button
            marginRight={tag && tag.marginRight}
            width={tag && tag.width}
            onClick={() => this.onClickButton(1000000)}
          >
