import { ButtonNormal } from "../../Commons/Button";
...
const AddPic = styled.div`
  ...
  margin-right: ${(props) =>
    props.marginRight === null ? 0 : props.marginRight}px;
...
export default class ContactModifyModal extends React.Component {
  closeModal = () => {
    this.props.close();
  };
  constructor(props) {
    ...
    this.state = {
      files: [],
      thumbnail:
        this.props.detail.contact_image_list &&
        this.props.detail.contact_image_list.split(","),
      ...this.props.detail,
      //             create_at: "2022-07-14T14:51:07.000Z"
      // exp_name: undefined
      // nick_name: "깜상"
      // option: null
      // contact_image_list: "https://s3.ap-northeast-2.amazonaws.com/osd.uploads.com/dev/uploads/c0d3a03a4f41d3b662d176e8f06b9ac9"
      // score: 2
      // text: "a"
      // uid: 29
      // user_id: 10
  ...
  onSubmit = async () => {
    ...
    Promise.all(
      ...
      .then((contact_image_list) => {
        console.log(contact_image_list);
        const obj = {
          user_id: userInfo.uid,
          exp_id: expDetail.uid,
          score: score < 6 ? score : parseInt(score / 20),
          text: text,
          contact_image_list: contact_image_list,
        };
  ...
  handleOnChangeThumbnail(event, index) {
    ...
    reader.onloadend = () => {
      ...
      if (this.state.thumbnail && this.state.thumbnail.length <= 0) {
        this.setState({
          thumbnail: this.state.thumbnail.concat(thumbnail_url),
          files: this.state.files.concat(file),
        });
      } else {
        let thumbnail_list =
          typeof this.state.thumbnail === "object"
            ? this.state.thumbnail
            : this.state.thumbnail.split(",");
        let file_list = this.state.files;
        thumbnail_list.splice(index, 1, thumbnail_url);
        file_list.splice(index, 1, file);
        this.setState({ thumbnail: thumbnail_list, files: file_list });
        ...
        <Wrapper>
          ...
          <div className="row">
            ...
            <div className="mini_pic_list">
                ...
                ? this.state.thumbnail.map((item, index) => (
                    <React.Fragment>
                      <input
                        hidden
                        onChange={(event) =>
                          this.handleOnChangeThumbnail(event, index)
                        }
                        id={`file${index}`}
                        type="file"
                        accept="image/*"
              ...
              {this.state.thumbnail &&
              this.state.thumbnail.length >= 5 ? null : (
                <React.Fragment>
                  <input
                    hidden
                    onChange={(event) =>
                      this.handleOnChangeThumbnail(
                        event,
                        this.state.thumbnail && this.state.thumbnail.length < 0
                          ? 0
